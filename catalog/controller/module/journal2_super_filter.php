        ...
        if ($module_data['category'] != 'off') {
            ...
            foreach ($results as $result) {
                $this->data['categories'][] = array(
                    ...
                    'image'         => $this->model_tool_image->resize($result['image'] ? $result['image'] : (Front::$IS_OC2 ? 'placeholder.png' : 'no_image.jpg'), $this->data['cat_image_width'], $this->data['cat_image_height']),
        ...
        if ($module_data['manufacturer'] !== 'off' && $data['route'] !== 'product/manufacturer/info') {
            ...
            foreach ($results as $result) {
                $this->data['manufacturers'][] = array(
                    ...
                    'name'			    => trim($result['name']) . ( $show_product_count ? ' (' . $result['total'] . ')' : ''),
                    'image'             => $this->model_tool_image->resize($result['image'] ? $result['image'] : (Front::$IS_OC2 ? 'placeholder.png' : 'no_image.jpg'), $this->data['brand_image_width'], $this->data['brand_image_height']),
                    'keyword'			=> $this->keyword($result['name'])
                );
            }
        ...
        foreach ($results as $key => $result) {
            ...
            foreach ($result['values'] as $value) {
                $values[] = array(
                    ...
                    'image'				=> $this->model_tool_image->resize($value['image'] ? $value['image'] : (Front::$IS_OC2 ? 'placeholder.png' : 'no_image.jpg'), $this->data['option_image_width'], $this->data['option_image_height']),
